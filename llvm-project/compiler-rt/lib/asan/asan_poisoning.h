...
ALWAYS_INLINE void FastPoisonShadow(uptr aligned_beg, uptr aligned_size,
  ...
  if (value || SANITIZER_WINDOWS == 1 ||
    ...
    // CombiSan: setting shadow memory to non-zero.
    // the to-be-poisoned size is always a multiple of 
    // 'ASAN_SHADOW_GRANULARITY' here, so we can just set everything to 'ASan invalid'
    // without having to account for half-filled shadow granules.
...
ALWAYS_INLINE void FastPoisonShadowPartialRightRedzone(
  ...
  // CombiSan: this is the function relevant for "partially valid" redzone starts
  // 'size' is the number of valid bytes in this chunk
  ...
  for (uptr i = 0; i < redzone_size; i += ASAN_SHADOW_GRANULARITY, shadow++) {
    ...
    if (i + ASAN_SHADOW_GRANULARITY <= size) {
      ...
      *shadow = kCombiASanRZ; // fully unaddressable
    } else {
      // first size-i bytes are addressable
      *shadow = poison_partial ? LookupPartial[size - i] : 0;
    }
#else
    if (i + ASAN_SHADOW_GRANULARITY <= size) {
      *shadow = 0;  // fully addressable
