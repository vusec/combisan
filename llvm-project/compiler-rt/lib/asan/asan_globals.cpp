...
ALWAYS_INLINE void PoisonRedZones(const Global &g) {
  // CombiSan: MSan: globals are always initialized (to zero or initializer)

  // CombiSan: global redzones
  uptr aligned_size = RoundUpTo(g.size, ASAN_SHADOW_GRANULARITY);
  FastPoisonShadow(g.beg + aligned_size, g.size_with_redzone - aligned_size,
    kCombiASanRZ); // was kAsanGlobalRedzoneMagic
  if (g.size != aligned_size) {
    // CombiSan: FastPoisonShadowPartialRightRedzone handles the partial redzone
...
static void PoisonDynamicGlobals(
  ...
  for (auto &dyn_g : dyn_globals) {
    ...
    if (dyn_g.initialized)
      continue;
    // CombiSan: poison dynamic globals with 0x55 (fully invalid)
    PoisonShadowForGlobal(g, kCombiASanRZ); // was kAsanInitializationOrderMagic
  }
}
...
void __asan_register_globals(__asan_global *globals, uptr n) {
  ...
  // CombiSan: poison global metadata with 0x55 (fully invalid)
