...
#    define ASAN_SHADOW_OFFSET_DYNAMIC
#  else
#    if ASAN_SHADOW_SCALE != 2
#      error "Value below is based on shadow scale = 2."
#      error "Original formula was: 0x7FFFFFFF & (~0xFFFULL << SHADOW_SCALE)."
#    endif
#    define ASAN_SHADOW_OFFSET_CONST 0x000000007fffc000
#  endif
#endif
...
/*
  ...
  --- ASan + MSan ---
  Fully valid, but uninitialized
  0xAA:   (10) (10) (10) (10)

  Fully invalid, initialization irrelevant!
  0x55:   (01) (01) (01) (01)
  ...
  0x6A:   (01) (10) (10) (10)
*/
// CombiSan: look-up table for # valid bytes to shadow bits (ASan-only)
static const u8 LookupPartial[4] = {kCombiASanRZ, kCombiASan13, kCombiASan22, kCombiASan31};
// CombiSan: look-up table for # valid bytes to shadow bits (with MSan invalidation)
// i.e.: the valid bytes are marked as uninitialized, while the others are redzones (b01)
static const u8 LookupPartialUninit[4] = {kCombiASanRZ, kCombiMSan13, kCombiMSan22, kCombiMSan31};
...
static inline bool AddressIsPoisoned(uptr a) {
  ...
  if (shadow_value) {
#if 1 // CombiSan: check the specific bit
    u64 bit_offset = (a & 0x3ULL) << 1ULL; // eq: (ptr%4)*2
    shadow_value = shadow_value >> bit_offset;
    u8 two_bit_val = shadow_value & 0x3;
    return two_bit_val != 0;
#else
...
static inline bool AddressIsPoisonedMasked(uptr a) {
  PROFILE_ASAN_MAPPING();
  u8 *shadow_address = (u8 *)MEM_TO_SHADOW(a);
  s8 shadow_value = *shadow_address;
  shadow_value &= kCombiSelectASan; // ASan-bits only
  if (shadow_value) {
    u64 bit_offset = (a & 0x3ULL) << 1ULL; // eq: (ptr%4)*2
    shadow_value = shadow_value >> bit_offset;
    u8 two_bit_val = shadow_value & 0x3;
...
static inline bool AddressIsPoisonedMaskAndInit(uptr a) {
  PROFILE_ASAN_MAPPING();
  // Printf("MSAN INIT: addr %p\n", a);
  u8 *shadow_address = (u8 *)MEM_TO_SHADOW(a);
  s8 shadow_value = *shadow_address;
  s8 shadow_value_masked = shadow_value & kCombiSelectASan; // ASan-bits only
  u64 bit_offset = (a & 0x3ULL) << 1ULL; // eq: (ptr%4)*2
  shadow_value_masked = shadow_value_masked >> bit_offset;
  u8 two_bit_val = shadow_value_masked & 0x3;
  // update shadow to initialized
  *shadow_address = shadow_value & ~(1 << (bit_offset+1));
  return two_bit_val != 0;
}

static inline bool AddressCopyAndCheckAligned(uptr from, uptr to) {
  // Printf("MSAN INIT: addr %p\n", a);

  u8* shadow_from = (u8 *)MEM_TO_SHADOW(from);
  u8* shadow_to = (u8 *)MEM_TO_SHADOW(to);

  u8 shadow_value_from = *shadow_from;
  u8 shadow_value_to = *shadow_to;

  u64 bit_offset = (from & 0x3ULL) << 1ULL; // from and to share alignment
  u8 two_bit_val_from = (shadow_value_from >> bit_offset) & 0x3;
  u8 two_bit_val_to = (shadow_value_to >> bit_offset) & 0x3;
  
  // copy shadow from -> to: first clear the old value
  shadow_value_to &= ~(1 << (bit_offset+1));
  // then insert the source value
  shadow_value_to |= (two_bit_val_from) << bit_offset;
  *shadow_to = shadow_value_to;
  // Printf("new shadow value at %p: %x (og from %x)\n", to, shadow_value_to, shadow_value_from);

  return two_bit_val_from & 1 || two_bit_val_to & 1; // check ASan bits only
}

static inline bool AddressCopyAndCheckUnaligned(uptr from, uptr to) {
  ...
  // from and to do not share alignment
  u64 bit_offset_from = (from & 0x3ULL) << 1ULL;
  u64 bit_offset_to = (to & 0x3ULL) << 1ULL;
  u8 two_bit_val_from = (shadow_value_from >> bit_offset_from) & 0x3;
  u8 two_bit_val_to = (shadow_value_to >> bit_offset_to) & 0x3;
  
  // copy shadow from -> to: first clear the old value
  shadow_value_to &= ~(1 << (bit_offset_to+1));
  // then insert the source value
  shadow_value_to |= (two_bit_val_from) << bit_offset_to;
  *shadow_to = shadow_value_to;
  // Printf("new shadow value at %p: %x (og from %x)\n", to, shadow_value_to, shadow_value_from);
...
// Copy MSan
static inline void AddressCopyAligned(uptr from, uptr to) {
...
// Copy MSan (unaligned)
static inline void AddressCopyUnaligned(uptr from, uptr to) {
