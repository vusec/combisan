...
struct QuarantineCallback {
  ...
  void PreQuarantine(AsanChunk *m) const {
    ...
    PoisonShadow(m->Beg(), RoundUpTo(m->UsedSize(), ASAN_SHADOW_GRANULARITY),
                  kCombiASanRZ); // was kAsanHeapFreeMagic
  }
  ...
  void Recycle(AsanChunk *m) const {
    ...
    if (get_allocator().FromPrimary(p)) {
      ...
      PoisonShadow(m->Beg(), RoundUpTo(m->UsedSize(), ASAN_SHADOW_GRANULARITY),
                  kCombiASanRZ); // was kAsanHeapLeftRedzoneMagic
    }
...
void AsanMapUnmapCallback::OnMap(uptr p, uptr size) const {
  PoisonShadow(p, size, kCombiASanRZ); // was kAsanHeapLeftRedzoneMagic
  // Statistics.
  AsanStats &thread_stats = GetCurrentThreadStats();
  thread_stats.mmaps++;
...
void AsanMapUnmapCallback::OnMapSecondary(uptr p, uptr size, uptr user_begin,
  ...
  PoisonShadow(p, user_begin - p, kCombiASanRZ); // was kAsanHeapLeftRedzoneMagic
  PoisonShadow(user_end, size - (user_end - p), kCombiASanRZ); // was kAsanHeapLeftRedzoneMagic
  // Statistics.
  AsanStats &thread_stats = GetCurrentThreadStats();
  thread_stats.mmaps++;
  thread_stats.mmaped += size;
...
struct Allocator {
  ...
  void RePoisonChunk(uptr chunk) {
    ...
    if (ac && atomic_load(&ac->chunk_state, memory_order_acquire) ==
      ...
      if (chunk < beg && beg < end && end <= chunk_end) {
        ...
        PoisonShadow(chunk, beg - chunk, kCombiASanRZ); // was kAsanHeapLeftRedzoneMagic
        uptr end_aligned_down = RoundDownTo(end, ASAN_SHADOW_GRANULARITY);
        FastPoisonShadowPartialRightRedzone(
            end_aligned_down, end - end_aligned_down,
            chunk_end - end_aligned_down, kCombiASanRZ); // was kAsanHeapLeftRedzoneMagic
        return;
      }
    }
    ...
    PoisonShadow(chunk, allocated_size, kCombiASanRZ); // was kAsanHeapLeftRedzoneMagic
  }
  ...
  void *Allocate(uptr size, uptr alignment, BufferedStackTrace *stack,
                 AllocType alloc_type, bool can_fill, bool set_init = true) {
    ...
    if (!from_primary || *(u8 *)MEM_TO_SHADOW((uptr)allocated) == 0) {
      ...
      PoisonShadow(alloc_beg, user_beg - alloc_beg, kCombiASanRZ); // was kAsanHeapLeftRedzoneMagic
      PoisonShadow(tail_beg, tail_end - tail_beg, kCombiASanRZ); // was kAsanHeapLeftRedzoneMagic
    }
    ...
    // CombiSan: no longer unpoison the true object, but in fact poison with MSan invalidation
    // if 'set_init' was set, it means we came from an alloc that is/should be (considered) initialized

    if (size_rounded_down_to_granularity)
      PoisonShadow(user_beg, size_rounded_down_to_granularity, set_init ? 0 : kCombiMSanUn); // was 0
    ...
    if (size != size_rounded_down_to_granularity && CanPoisonMemory()) {
      ...
      // CombiSan: here we need to adjust for the size (partial redzone, uninit valids).
      // XXX: we may want to restore 'fl.poison_partial'
      uptr valid_bytes_last_granule = size - size_rounded_down_to_granularity;
      *shadow = set_init ? LookupPartial[valid_bytes_last_granule] : LookupPartialUninit[valid_bytes_last_granule];
    }
  ...
  void *Reallocate(void *old_ptr, uptr new_size, BufferedStackTrace *stack, bool set_init = true) {
    ...
    void *new_ptr = Allocate(new_size, 8, stack, FROM_MALLOC, true, set_init);
    if (new_ptr) {
...
#if 1 // CombiSan
      // copy the initialization state from old_ptr to new_ptr.
      __asan_region_copy_nochk((uptr)old_ptr, (uptr)new_ptr, memcpy_size);
#endif
  ...
  void *Calloc(uptr nmemb, uptr size, BufferedStackTrace *stack, bool set_init = true) {
    if (UNLIKELY(CheckForCallocOverflow(size, nmemb))) {
      if (AllocatorMayReturnNull())
        return nullptr;
      ReportCallocOverflow(nmemb, size, stack);
    }
    // CombiSan: pass init=true to Allocate
    void *ptr = Allocate(nmemb * size, 8, stack, FROM_MALLOC, false, set_init);
...
void *asan_malloc(uptr size, BufferedStackTrace *stack, bool set_init) {
  return SetErrnoOnNull(instance.Allocate(size, 8, stack, FROM_MALLOC, true, set_init));
}

void *asan_calloc(uptr nmemb, uptr size, BufferedStackTrace *stack, bool set_init) {
  return SetErrnoOnNull(instance.Calloc(nmemb, size, stack, set_init));
}

void *asan_reallocarray(void *p, uptr nmemb, uptr size,
                        BufferedStackTrace *stack, bool set_init) {
  if (UNLIKELY(CheckForCallocOverflow(size, nmemb))) {
    errno = errno_ENOMEM;
    if (AllocatorMayReturnNull())
      return nullptr;
...
void *asan_realloc(void *p, uptr size, BufferedStackTrace *stack, bool set_init) {
  if (!p)
    return SetErrnoOnNull(instance.Allocate(size, 8, stack, FROM_MALLOC, true, set_init));
  if (size == 0) {
  ...
  return SetErrnoOnNull(instance.Reallocate(p, size, stack, set_init));
}

void *asan_valloc(uptr size, BufferedStackTrace *stack, bool set_init) {
  return SetErrnoOnNull(
      instance.Allocate(size, GetPageSizeCached(), stack, FROM_MALLOC, true, set_init));
}

void *asan_pvalloc(uptr size, BufferedStackTrace *stack, bool set_init) {
  ...
  return SetErrnoOnNull(
      instance.Allocate(size, PageSize, stack, FROM_MALLOC, true, set_init));
}

void *asan_memalign(uptr alignment, uptr size, BufferedStackTrace *stack,
                    AllocType alloc_type, bool set_init) {
  ...
  return SetErrnoOnNull(
      instance.Allocate(size, alignment, stack, alloc_type, true, set_init));
}

void *asan_aligned_alloc(uptr alignment, uptr size, BufferedStackTrace *stack, bool set_init) {
  ...
  return SetErrnoOnNull(
      instance.Allocate(size, alignment, stack, FROM_MALLOC, true, set_init));
}
...
int asan_posix_memalign(void **memptr, uptr alignment, uptr size,
  ...
  void *ptr = instance.Allocate(size, alignment, stack, FROM_MALLOC, true, set_init);
  if (UNLIKELY(!ptr))
