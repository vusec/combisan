...
ALWAYS_INLINE void SetShadow(uptr ptr, uptr size, uptr class_id, u64 magic) {
...
#if 1 // CombiSan: reason about ASAN_SHADOW_SCALE == 2
  if (ASAN_SHADOW_SCALE == 2 && class_id <= 6) {
    // This code expects ASAN_SHADOW_SCALE=3.
    // For scale=2, each u64 write covers 8 shadow bytes (8×4=32 app. bytes),
    // so we need twice as many iterations to cover the same app. size.
    for (uptr i = 0; i < (((uptr)1) << (class_id+1)); i++) {
      shadow[i] = magic;
  ...
  if (ASAN_SHADOW_SCALE == 3 && class_id <= 6) {
    // This code expects ASAN_SHADOW_SCALE=3.
    // For scale=3, each u64 write covers 8 shadow bytes (8×8=64 app bytes)
    ...
    for (uptr i = 0, n = NumberOfFrames(stack_size_log(), class_id); i < n;
      ...
      if (bottom < ff->real_stack && ff->real_stack < real_stack) {
        ...
        SetShadow(reinterpret_cast<uptr>(ff), BytesInSizeClass(class_id),
                  class_id, kCombiSelectASan8B); // was kMagic8 (kAsanStackAfterReturnMagic)
      }
    }
  }
  needs_gc_ = false;
...
static ALWAYS_INLINE void OnFree(uptr ptr, uptr class_id, uptr size) {
  ...
  SetShadow(ptr, size, class_id, kCombiSelectASan8B); // CombiSan was kMagic8
}
...
void __asan_alloca_poison(uptr addr, uptr size) {
  ...
  FastPoisonShadow(LeftRedzoneAddr, kAllocaRedzoneSize, kCombiASanRZ); // was kAsanAllocaLeftMagic
  FastPoisonShadowPartialRightRedzone(
      PartialRzAligned, PartialRzAddr % ASAN_SHADOW_GRANULARITY,
      RightRzAddr - PartialRzAligned, kCombiASanRZ); // was kAsanAllocaRightMagic
  FastPoisonShadow(RightRzAddr, kAllocaRedzoneSize, kCombiASanRZ); // was kAsanAllocaRightMagic
}
