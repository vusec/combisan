...
// CombiSan profiling
u64 MSanViolationCnt = 0;
u64 MSanMappy[MSAN_MAX_MAP] = {0};
#endif

// u8* combisan_area_ptr;
...
static void OnLowLevelAllocate(uptr ptr, uptr size) {
  PoisonShadow(ptr, size, kCombiASanRZ); // was kAsanInternalHeapMagic
}
...
void __asan_loadN(uptr addr, uptr size) {
  // CombiSan: loading unusual size N at addr.
  // here we also check for MSan bits
...
#if 1 // CombiSan
  // store operation: check ASan, initialize MSan
  if ((addr = __asan_region_chk_poisoned_set_init(addr, size))) {
    GET_CALLER_PC_BP_SP;
    ReportGenericError(pc, bp, sp, addr, true, size, 0, true, true);
  }
#else
  if ((addr = __asan_region_is_poisoned(addr, size))) {
    GET_CALLER_PC_BP_SP;
...
void __asan_storeN_noabort(uptr addr, uptr size) {
  ...
  if ((addr = __asan_region_chk_poisoned_set_init(addr, size))) {
    GET_CALLER_PC_BP_SP;
    ReportGenericError(pc, bp, sp, addr, true, size, 0, false, true);
  }
#else
  if ((addr = __asan_region_is_poisoned(addr, size))) {
    GET_CALLER_PC_BP_SP;
...
extern "C"
NOINLINE INTERFACE_ATTRIBUTE
void __combisan_initialize(uptr addr, uptr size) {
  FastPoisonShadow(addr, size, 0x00); // set to init
}
...
static NOINLINE void force_interface_symbols() {
  ...
  switch (fake_condition) {
    ...
    case 40:__asan_set_shadow_00(0, 0); break;
    case 41:__asan_set_shadow_6a(0, 0); break;
    case 42:__asan_set_shadow_5a(0, 0); break;
    case 43:__asan_set_shadow_56(0, 0); break;
    case 44:__asan_set_shadow_aa(0, 0); break;
    case 45:__asan_set_shadow_40(0, 0); break;
    case 46:__asan_set_shadow_50(0, 0); break;
    case 47:__asan_set_shadow_54(0, 0); break;
    case 48:__asan_set_shadow_55(0, 0); break;
...
#ifdef COMBISAN_PROFILING
static void combisan_atexit() {
  Printf("CombiSan exit stats:\n");
  Printf("CoMSanViolationCnt: %lu\n", MSanViolationCnt);
  u64 UniqueMappers = 0;
  for(u64 i = 0; i < MSAN_MAX_MAP; i++){
    if(MSanMappy[i] != 0){
      UniqueMappers++;
    }
  }
  Printf("CoUniqueMappers: %lu\n", UniqueMappers);
}
#endif
...
#ifdef COMBISAN_PROFILING
  // CombiSan: natural and die exits: print some stats
  Atexit(combisan_atexit);
  AddDieCallback(combisan_atexit);
#endif

// XXXauth: Load map
#if 0
  char *combisan_id_str = getenv(COMBISAN_SHM_ENV_VAR);
  if (combisan_id_str) {
    int msan_shm_id = atoi(combisan_id_str);
    combisan_area_ptr = (u8*) shmat(msan_shm_id, NULL, 0);
    if (combisan_area_ptr == (void *)-1){ 
      Printf("COMBISAN_SHM_ENV_VAR detected with ID %d,"
                    " but shmat() failed. Exiting.\n", combisan_id_str);
      exit(1);
    }
  }
#endif
...
// these asan allocators explicitly set the MSan initialization
// state to false (so uninitialized)
// these are used for the program allocators

extern "C" INTERFACE_ATTRIBUTE void *__combisan_ind_get_handler(void *fn){
  if(!fn) return fn;

  if(fn == &malloc){
    return (void*)&asan_malloc_uninit;
  }
  if(fn == &calloc){
    return (void*)&asan_calloc_uninit;
  }
  if(fn == &realloc){
    return (void*)&asan_realloc_uninit;
  }
  if(fn == &posix_memalign){
    return (void*)&asan_posix_memalign_uninit;
  }
  if(fn == &memalign){
    return (void*)&asan_memalign_uninit;
  }

  // TODO: is it possible to do an indirect call to 'new'?
  // TODO: reallocarray/valloc/pvalloc/__libc_memalign/aligned_alloc (if relevant...)

  // indirect calls to deallocators (free) we do not have to capture
  // they are simply deallocated by the standard free override
  // because free is MSan-agnostic (shadow state becomes ASan invalid)

  // not a known allocator, just call fn
  return fn;
}

void* asan_malloc_uninit(uptr size){
  GET_STACK_TRACE_MALLOC;
  return asan_malloc(size, &stack, false); // inited = false
}

void* asan_calloc_uninit(uptr nmemb, uptr size){
  GET_STACK_TRACE_MALLOC;
  return asan_calloc(nmemb, size, &stack, true); // inited = true, calloc we know is initialized
}

void *asan_realloc_uninit(void *p, uptr size){
  GET_STACK_TRACE_MALLOC;
  return asan_realloc(p, size, &stack, false); // inited = false
}

void* asan_reallocarray_uninit(void *ptr, uptr nmemb, uptr size) {
  GET_STACK_TRACE_MALLOC;
  return asan_reallocarray(ptr, nmemb, size, &stack, false); // inited = false
}

void* asan_memalign_uninit(uptr boundary, uptr size) {
  GET_STACK_TRACE_MALLOC;
  return asan_memalign(boundary, size, &stack, FROM_MALLOC, false); // inited = false
}

void* asan___libc_memalign_uninit(uptr boundary, uptr size) {
  GET_STACK_TRACE_MALLOC;
  return asan_memalign(boundary, size, &stack, FROM_MALLOC, false); // inited = false
}

void* asan_aligned_alloc_uninit(uptr boundary, uptr size) {
  GET_STACK_TRACE_MALLOC;
  return asan_aligned_alloc(boundary, size, &stack, false); // inited = false
}

int asan_posix_memalign_uninit(void **memptr, uptr alignment, uptr size) {
  GET_STACK_TRACE_MALLOC;
  return asan_posix_memalign(memptr, alignment, size, &stack, false); // inited = false
}

void* asan_valloc_uninit(uptr size) {
  GET_STACK_TRACE_MALLOC;
  return asan_valloc(size, &stack, false); // inited = false
}

void* asan_pvalloc_uninit(uptr size) {
  GET_STACK_TRACE_MALLOC;
  return asan_pvalloc(size, &stack, false); // inited = false
}

void *asan_new_uninit(size_t size) {
  GET_STACK_TRACE_MALLOC;
  void *res = asan_memalign(0, size, &stack, FROM_NEW, false); // inited = false
  if (UNLIKELY(!res))
    ReportOutOfMemory(size, &stack);
  return res;
}

void *asan_new_arr_uninit(size_t size) {
  GET_STACK_TRACE_MALLOC;
  void *res = asan_memalign(0, size, &stack, FROM_NEW_BR, false); // inited = false
  if (UNLIKELY(!res))
    ReportOutOfMemory(size, &stack);
  return res;
}

void *asan_new_thr_uninit(size_t size, std::nothrow_t const&) {
  GET_STACK_TRACE_MALLOC;
  void *res = asan_memalign(0, size, &stack, FROM_NEW, false); // inited = false
  return res;
}

void *asan_new_thr_arr_uninit(size_t size, std::nothrow_t const&) {
  GET_STACK_TRACE_MALLOC;
  void *res = asan_memalign(0, size, &stack, FROM_NEW_BR, false); // inited = false
  return res;
}

void *asan_new_aln_uninit(size_t size, std::align_val_t align) {
  GET_STACK_TRACE_MALLOC;
  void *res = asan_memalign((uptr)align, size, &stack, FROM_NEW, false); // inited = false
  if (UNLIKELY(!res))
    ReportOutOfMemory(size, &stack);
  return res;
}

void *asan_new_arr_aln_uninit(size_t size, std::align_val_t align) {
  GET_STACK_TRACE_MALLOC;
  void *res = asan_memalign((uptr)align, size, &stack, FROM_NEW_BR, false); // inited = false
  if (UNLIKELY(!res))
    ReportOutOfMemory(size, &stack);
  return res;
}

void *asan_new_thr_aln_uninit(size_t size, std::align_val_t align, std::nothrow_t const&) {
  GET_STACK_TRACE_MALLOC;
  void *res = asan_memalign((uptr)align, size, &stack, FROM_NEW, false); // inited = false
  return res;
}

void *asan_new_thr_aln_arr_uninit(size_t size, std::align_val_t align, std::nothrow_t const&) {
  GET_STACK_TRACE_MALLOC;
  void *res = asan_memalign((uptr)align, size, &stack, FROM_NEW_BR, false); // inited = false
  return res;
}
