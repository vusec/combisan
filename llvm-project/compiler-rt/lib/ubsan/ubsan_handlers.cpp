...
#define COMBISAN_LOG_UBSAN_BUG(pc)                                            \
                            do{                                               \
                              if(combisan_area_ptr){                          \
                                  combisan_metadata *ptr =                    \
                                    (combisan_metadata*) combisan_area_ptr;   \
                                  u64 idx = pc % AFL_MAP_SIZE;                \
                                                                              \
                                  if(idx<sizeof(combisan_metadata))           \
                                    idx = sizeof(combisan_metadata);          \
                                                                              \
                                  if(!(combisan_area_ptr[idx] & UBSAN_BUG)){  \
                                    combisan_area_ptr[idx] |= UBSAN_BUG;      \
                                    ptr->ubsan_new = 1;                       \
                                  }                                           \
                              }                                               \
                              if(combisan_dbg){                               \
                                Printf("UBSan bug in %p\n", pc);              \
                              }                                               \
                            } while(0);                                       
#else
