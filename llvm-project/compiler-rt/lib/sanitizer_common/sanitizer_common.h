...
//XXXauth: we need this to load the violation map. We cannot do it with 
//          the initialization of asan as it happens too early, and the
//          env variables have not been loaded yet.
#if 1
#include <stdint.h>
...
extern uint8_t* combisan_area_ptr;
extern bool combisan_dbg;

// map size
#define AFL_MAP_SIZE_POW2     16
#define AFL_MAP_SIZE          (1U << AFL_MAP_SIZE_POW2)

//XXXauth: this is very unholy, we are using the same definition in AFL and
//          expect the compilers to compile it in the same way (likely, but
//          we should be safe and define an array to which we assign fields)
#ifdef COMBISAN_DEFERRED
#define COMBISAN_ASAN_DEFERRED
#define COMBISAN_MSAN_DEFERRED
typedef struct{
...
#elif defined(COMBISAN_OPPORTUNISTIC)
#define COMBISAN_ASAN_OPPORTUNISTIC
#define COMBISAN_MSAN_OPPORTUNISTIC
typedef struct{
  uint64_t msan_last_edge;
...
#define MSAN_CS           (1 << 2)
#define ASAN_PC           (1 << 3)
#define ASAN_EDGE         (1 << 4)
#define ASAN_CS           (1 << 5)
#define UBSAN_BUG         (1 << 6)

#elif defined(COMBISAN_MIXED)
#define COMBISAN_ASAN_DEFERRED
#define COMBISAN_MSAN_OPPORTUNISTIC
typedef struct{
  uint64_t msan_last_edge;
...
__attribute__((constructor)) void combisan_shmem_init(void);
#endif 
