...
bool mem_is_zero(const char *beg, uptr size) {
  ...
  u32 *aligned_beg = (u32 *)RoundUpTo((uptr)beg, sizeof(u32));
...
bool mem_is_zero_masked(const char *beg, uptr size) {
  CHECK_LE(size, 1ULL << FIRST_32_SECOND_64(30, 40));  // Sanity check.
  const char *end = beg + size;
  u32 *aligned_beg = (u32 *)RoundUpTo((uptr)beg, sizeof(u32));
  u32 *aligned_end = (u32 *)RoundDownTo((uptr)end, sizeof(u32));
  ...
  // Prologue.
  for (const char *mem = beg; mem < (char*)aligned_beg && mem < end; mem++)
    all |= *mem & 0x55; // only select ASan bits
  // Aligned loop.
  for (; aligned_beg < aligned_end; aligned_beg++)
    all |= *aligned_beg & 0x55555555UL; // only select ASan bits
  // Epilogue.
  if ((char *)aligned_end >= beg) {
    for (const char *mem = (char *)aligned_end; mem < end; mem++)
      all |= *mem & 0x55;
