...
#include <sys/types.h>
#include <sys/shm.h>
uint8_t* combisan_area_ptr;
bool combisan_dbg = false;

__attribute__((constructor)) void combisan_shmem_init(){

  char *combisan_id_str = getenv(COMBISAN_SHM_ENV_VAR);
  if (combisan_id_str) {
    int msan_shm_id = atoi(combisan_id_str);
    combisan_area_ptr = (uint8_t*) shmat(msan_shm_id, NULL, 0);
    if (combisan_area_ptr == (void *)-1){ 
      exit(42);
    }
  }

  //XXXauth: have a way to print violations
  if(getenv("COMBISAN_PRINT_VIOLATIONS")){
    combisan_dbg = true;
  }
}
#endif
