...
extern char *asan_dir;
extern char *ubsan_dir;

//accurate detection
//for some valgrind versions you may need to add expensive checks manually
//        --expensive-definedness-checks=yes
//timeout is added to avoid pathologic cases to stop fuzzing forever
#define valgrind_path   "timeout 10 valgrind --error-exitcode=42 --exit-on-first-error=yes"
extern char *accurate_cmdline;
#endif
...
#ifdef COMBISAN
      u8  *combisan_traces;                 /* CombiSan coverage of violations  */
          //MSan
      u64 acurate_runs,                     /* Total execution with accurate det*/
          UUM_errors,                       /* Total numbre of UUM errors       */
          combisan_msan_pc,                 /* Total number of faulty inst.     */
          combisan_msan_edges,              /* Total edges bytes in bitmap      */  
          combisan_msan_callstack,          /* Total callstack bytes in bitmap  */  
          //ASan
          asan_crahses,                     /* Total ASan crashes (before dedup)*/
          combisan_asan_pc,                 /* Total number of faulty inst.     */
          combisan_asan_edges,              /* Total edges bytes in bitmap      */  
          combisan_asan_callstack,          /* Total callstack bytes in bitmap  */
          //UBSan
          ubsan_crashes;                    /* Total UBSan crashes (bef dedup)  */  
      int combisan_log_fd;
      int valgrind_log_fd;
#endif
...
#ifdef COMBISAN
void combisan_write_bitmap(afl_state_t *);
#endif
...
#ifdef COMBISAN
int CombiSan_check_bugs(afl_state_t*);
#endif
