...
//XXXauth: we need the same for MSan
__attribute__((weak)) void __msan_unpoison(const volatile void *a, size_t size){

  (void)a;
  (void)size;

}
__attribute__((weak)) void __msan_poison(const volatile void *a, size_t size){
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (length > 0) {
      ...
      if (length < prev_length) {
        ...
        __msan_poison(buf + length, prev_length - length);

      } else {

        __asan_unpoison_memory_region(buf + prev_length, length - prev_length);
        __msan_unpoison(buf + prev_length, length - prev_length);
