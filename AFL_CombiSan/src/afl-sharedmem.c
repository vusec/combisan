...
#ifdef COMBISAN
//combisan shared memory cleaning
void combisan_shm_deinit(sharedmem_t *shm) {

  if (shm == NULL) { return; }

  unsetenv(COMBISAN_SHM_ENV_VAR);

  shmctl(shm->shm_id, IPC_RMID, NULL);

  shm->map = NULL;

  OKF("[CombiSan] shmem destroyed with id: %d", shm->shm_id);

}

//combisan shared memory creation
u8 *combisan_shm_init(sharedmem_t* shmem) {
  ...
  if (id < 0) {
    ...
    PFATAL("[CombiSan]shmget() failed, try running afl-system-config");

  }

  shm_str = alloc_printf("%d", id);

  setenv(COMBISAN_SHM_ENV_VAR, shm_str, 1);
  ...
  if (ptr == (void *)-1 || !ptr)
    PFATAL("[CombiSan]shmat() failed");

  shmem->shm_id = id;
  shmem->map = ptr;

  OKF("[CombiSan] shmem created with id: %d", id);
