...
int main(int argc, char **argv_orig, char **envp) {
...
#ifdef COMBISAN
  //CombiSan initialization

  //CombiSan coverage
  afl->combisan_traces = ck_alloc(MAP_SIZE);

  //shared memory for the PUT
  sharedmem_t* combisan_shm = calloc(1, sizeof(sharedmem_t));
  afl->fsrv.combisan_bits = combisan_shm_init(combisan_shm);
  char* combisan_log = alloc_printf("%s/%s", afl->out_dir, "combisan_log");
  afl->combisan_log_fd = open(combisan_log,
                              O_WRONLY | O_APPEND | O_CREAT | O_TRUNC, 0644);
  if(afl->combisan_log_fd == -1){
    BADF("Error while creating CombiSan's logging file\n");
    exit(1);
  }

  //store combisan's violations
  violations = alloc_printf("%s/%s", afl->out_dir, "violations");
  mkdir(violations, 0777);

  //store combisan's ASan bugs
  asan_dir = alloc_printf("%s/%s", afl->out_dir, "asan");
  mkdir(asan_dir, 0777);

  //store combisan's UBSan bugs
  ubsan_dir = alloc_printf("%s/%s", afl->out_dir, "ubsan");
  mkdir(ubsan_dir, 0777);

  //Test the accurate detector
  if(getenv("COMBISAN_UNINSTRUMENTED")){
    ACTF("[CombiSan] Uninstrumented binary provided, let's test it.");
    char *uninstrumented = getenv("COMBISAN_UNINSTRUMENTED");

    //yeah, this is the kind of bugs we are trying to look for :)
    char aux[512];
    sprintf(aux, "%s %s >/dev/null 2>&1 ", valgrind_path, "--version");
    int status = system(aux);
    if(WEXITSTATUS(status)){
      BADF("[CombiSan] Error while trying to invoke valgrind (exit code: %d)", status);
      exit(1);
    }
    OKF("[CombiSan] Valgrind works fine.");

    //Save all the output from valgrind so that we don't need to replay later

    char* valgrind_log = alloc_printf("%s/%s", afl->out_dir, "valgrind_log");
    afl->valgrind_log_fd = open(valgrind_log,
                                O_WRONLY | O_APPEND | O_CREAT | O_TRUNC, 0644);
    if(afl->valgrind_log_fd == -1){
      BADF("Error while creating valgrind's logging file\n");
      exit(1);
    }

    //quick and dirty: create an input...
    system("echo \"hi\" > combisan_tmp");
    //in the first one it is okay to overwrite, in case there is something in it
    sprintf(aux, "AFL_DISABLE_LLVM_INSTRUMENTATION=1 %s %s %s > %s 2>&1 ",
            valgrind_path, uninstrumented, "combisan_tmp", valgrind_log);
    status = system(aux);
    if(WEXITSTATUS(status)){
      BADF("[CombiSan] Error while trying to run binary on valgrind (exit code: %d)", status);
      BADF("[CombiSan] Command line: %s", aux);
      exit(1);
    }
    OKF("[CombiSan] Uninstrumented binary seems to work as well!");
    //... and remove it
    system("rm combisan_tmp");

    //Let's create the format string to ease accurate detector's invocations
    //use AFL_DISABLE_LLVM_INSTRUMENTATION to avoid AFL binaries to interact with maps
    accurate_cmdline = alloc_printf("AFL_DISABLE_LLVM_INSTRUMENTATION=1 %s %s %s/%%d >> %s 2>&1 \n",
                        valgrind_path, uninstrumented, violations, valgrind_log);
    ACTF("[CombiSan] Accurate detector uses the following commandline:");
    //0 is just a random number: the "%d" needs something to parse.
    SAYF(accurate_cmdline, 0);
  }
#endif
  if (!afl->non_instrumented_mode && !afl->fsrv.qemu_mode &&
  ...
  //Here we can start checking the slowdown
  u64 slowdown_start_ts;
  if(getenv("SLOWDOWN_TEST"))
    slowdown_start_ts = get_cur_time();
...
stop_fuzzing:
  ...
  //Here it ends
  if(getenv("SLOWDOWN_TEST")){
    u64 slowdown_ts = get_cur_time() - slowdown_start_ts;
    ACTF("\n\nSlowdown test took %llu ms.\n", slowdown_ts);
    char* slowdown_test = alloc_printf("%s/%s", afl->out_dir, "slowdown_test");
    int slowdown_test_fd = open(slowdown_test,
                                O_WRONLY | O_APPEND | O_CREAT | O_TRUNC, 0644);
    if(slowdown_test_fd == -1){
      SAYF("Could not create slowdown_test file.");
    }
    dprintf(slowdown_test_fd, "%llu", slowdown_ts);
    close(slowdown_test_fd);
  }
  #ifdef COMBISAN
    combisan_write_bitmap(afl);
  #endif
  save_auto(afl);
...
ck_free(afl->combisan_traces);

//forkserver shm cleaning
combisan_shm_deinit(combisan_shm);
close(afl->combisan_log_fd);

//deallocate cmdline string, if any
if(accurate_cmdline)
  ck_free(accurate_cmdline);
#endif
