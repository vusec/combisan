...
void write_stats_file(afl_state_t *afl, u32 t_bytes, double bitmap_cvg,
  ...
  fprintf(f,
          ...
          "command_line      : %s\n"
#ifdef COMBISAN
          //TODO: remove the properties we do not use
          "***CombiSan stats***\n"
          "UUM:"
          "\tcrashes       : %llu\n"
          "\taccurate runs : %llu\n"
          ...
          afl->orig_cmdline
#ifdef COMBISAN
          ,afl->UUM_errors, afl->acurate_runs, afl->combisan_msan_pc, afl->combisan_msan_edges,
          afl->combisan_msan_callstack, afl->asan_crahses, afl->combisan_asan_pc,
          afl->combisan_asan_edges, afl->combisan_asan_callstack, afl->ubsan_crashes
#endif
          );
